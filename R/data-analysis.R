...
# df_mm_highway_morning <- df_mm_road_type |> 
#   filter(id %in% c(2),
...
#          id == 90) 
# 
# df_mm_morning <- df_mm_road_type |> 
#   filter(id %in% c(1,2,5)) |> 
#   drop_na("aae_blue_ir") |> 
#   bind_rows(df_pm_aae_ver) |> 
#   mutate(root_source = ifelse(aae_blue_ir < 1.29, "ff",
#                               ifelse(aae_blue_ir > 1.64, "bb", "mixed"))) |> 
#   group_by(settlement_id,
#            root_source) |>
#   summarise(count = n()) |> 
#   pivot_wider(names_from = root_source, values_from = count)
# 
# df_mm_morning |> 
# ggplot(aes(x = settlement_id, y = aae_blue_ir, group = settlement_id)) +
#   geom_boxplot() +
#   geom_hline(yintercept = 1.63, linetype = "dashed", color = "brown") +
#   geom_hline(yintercept = 1.29, linetype = "dashed", color = "black") +
#   theme
# 
# df_mm_ver |>
...
# 
# colors_manual <- c('black',  'darkblue', "brown")
# #
...
#           breaks = c(0.7, 1.29, 1.64, 2.35),
#           palette = colors_manual,
#           title = "AAE (475/880nm)") 
#   #tm_facets(by = "settlement_id", nrow = 2, free.coords = FALSE)
# 
# aae_map
# 
# df_ndirande_zoom <- tibble(
#   lat = c(-15.765, -15.773),
#   long = c(35.04, 35.045))
# 
# # df_map_ndirande <- df_map |> 
...
# #          long < 35.045) 
# 
# # map_data_ndirande <- st_as_sf(df_map_ndirande, coords = c("long", "lat"))
# # 
# # ggplot() +
# #   base_map(st_bbox(map_data_ndirande), basemap = "mapnik", increase_zoom = 3) +
# #   geom_path(data = df_map, 
# #             aes(x = long, 
# #                 y = lat, 
# #                 color = root_source), 
